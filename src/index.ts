...
client.once('ready', async () => {
    ...
    const commands = [
        ...
        new SlashCommandBuilder().setName('skip').setDescription('進行中のフェーズを早めたい時に使います')
            .addSubcommand(s => s.setName('talk').setDescription('議論(昼)フェーズのスキップに賛成の投票をします（生存者の過半数で成立）')),
        ...
        try {
            switch (interaction.commandName) {
                ...
                case 'skip': {
                    ...
                    if (subCmd === 'talk') {
                        if (!hasGame(channel.id)) return interaction.reply({ content: '⚠️ このチャンネルにはゲームが存在しません。', ephemeral: true });
                        const game = getGame(channel.id);
                        const result = Phase.registerDiscussionSkipVote(game, interaction.user.id);
                        // 先に「成立しました」replyを完全に送り終えてから、投票フェーズへの移行を発火させる（メッセージの前後関係を保証するため）
                        await interaction.reply({ content: result.message, ephemeral: !result.success });
                        if (result.achieved) Phase.triggerDiscussionSkip(game);
                    }
                    return;
                }
