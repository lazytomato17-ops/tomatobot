...
import { Client, GatewayIntentBits, Interaction, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, GuildMember, SlashCommandBuilder, TextChannel, PermissionFlagsBits } from 'discord.js';
    ...
    cron.schedule('0 0 1 * *', async () => {
        ...
        try {
            ...
            guild.members.cache.forEach(async m => {
                if (m.roles.cache.has(RATE_TOP_ROLE_ID)) await m.roles.remove(RATE_TOP_ROLE_ID).catch(() => {});
            });
            if (rankers.topRate?.length) {
                const m = await guild.members.fetch(rankers.topRate[0].id).catch(() => null);
                if (m) await (m as any).roles.add(RATE_TOP_ROLE_ID).catch(() => {});
            }
            ...
            switch (interaction.commandName) {
                ...
                case 'setup_verify': {
                    ...
                    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
                        new ButtonBuilder().setCustomId(`verify_role_${role.id}`).setLabel('認証する / Verify').setStyle(ButtonStyle.Success).setEmoji('✅')
                    );
                ...
                case 'kick': {
                    ...
                    if (result.success && hasGame(channel.id)) {
                        const game = getGame(channel.id);
                        if (game.lobbyMessage) await game.lobbyMessage.edit(await Messages.getLobbyPayload(game, game.hostId, null)).catch(() => {});
                    }
                ...
                case 'sysinfo': {
                    ...
                    const formatUptime = (s: number) => `${Math.floor(s / 86400)}日 ${Math.floor(s % 86400 / 3600)}時間 ${Math.floor(s % 3600 / 60)}分`;
                    ...
                    await interaction.editReply({ embeds: [
                        new EmbedBuilder()
                            ...
                            .addFields(
                                ...
                                { name: `${memEmoji} メモリ`,  value: `${usedMB}MB / ${limitMB}MB (${pct}%)`, inline: true },
                ...
                case 'penalty': {
                    ...
                    if (res.success) {
                        await interaction.editReply({ embeds: [new EmbedBuilder().setTitle('🚨 【運営制裁の執行】').setDescription(`**対象者:** ${targetUser}\n**内容:** ${res.message}\n**理由:** ${reason}`).setColor(0x000000)] });
                    } else {
                ...
                case 'preset': {
                    ...
                    if (!hasGame(channel.id)) return interaction.reply({ content: '⚠️ 募集ロビーが開いている最中にのみ使用できます。', ephemeral: true });
                    const game = getGame(channel.id);
                    if (game.state !== 'recruiting') return interaction.reply({ content: '⚠️ 募集ロビーが開いている最中にのみ使用できます。', ephemeral: true });
                    if (game.hostId !== userId) return interaction.reply({ content: '⚠️ ホストのみ実行できます。', ephemeral: true });
                    ...
                    if (sub === 'load') {
                        ...
                        if (game.lobbyMessage) await game.lobbyMessage.edit(await Messages.getLobbyPayload(game, userId, interaction.member as any)).catch(() => {});
