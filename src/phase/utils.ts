...
export function setSafeTimeout(game: GameState, callback: () => void, ms: number): NodeJS.Timeout {
    ...
    return timer; // 👈 早期キャンセルしたい呼び出し元のために返り値を追加（既存の呼び出し箇所は返り値を無視するだけなので影響なし）
}
