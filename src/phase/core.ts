...
/**
 * /skip talk コマンドのハンドラ。議論(昼)フェーズ中の生存プレイヤーが呼び出すことを想定している。
 * 生存している人間プレイヤーの過半数が賛成した時点で、議論フェーズを即座に終了して投票フェーズへ進める。
 */
export function requestDiscussionSkip(game: GameState, playerId: string): { success: boolean; message: string } {
    if (game.state !== 'playing' || !game.isDiscussionActive || !game.discussionCollector) {
        return { success: false, message: MSG.day.skipNotAvailable };
    }
    ...
    if (game.discussionSkipVotes.includes(playerId)) {
        return { success: false, message: MSG.day.skipAlreadyVoted };
    }
    ...
    if (current >= needed) {
        ...
        game.discussionCollector = null;
        try { collector?.stop(DISCUSSION_SKIP_REASON); } catch (e) { console.error('[requestDiscussionSkip] collector.stop エラー:', e); }
        return { success: true, message: fill(MSG.day.skipAchieved, { current, needed }) };
    }

    return { success: true, message: fill(MSG.day.skipRequested, { name: player.name, current, needed }) };
}
