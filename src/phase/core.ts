...
// /skip talk によって議論コレクターを早期停止した際に 'end' イベントへ渡す理由文字列。
// 通常のタイムアウト終了と区別し、終了メッセージの重複送信を防ぐために使用する。
const DISCUSSION_SKIP_REASON = 'discussion_skip';
...
export async function startDayPhase(game: GameState) {
    ...
    game.isDiscussionActive = true;
    game.discussionSkipVotes = [];
    ...
    game.discussionCollector = msgCollector;
...
// src/phase.ts 内の startDayPhase 関数の一部を書き換え
    ...
    // ⏰ 議論終了◯秒前の予告アナウンス。/skip talk で早期終了した場合はこのタイマーも一緒に破棄する
    let warningTimer: NodeJS.Timeout | null = null;
    if (duration * 1000 > TIMING.discussionWarningBefore) {
        warningTimer = setSafeTimeout(game, () => {
            if (game.state !== 'playing' || !game.isDiscussionActive) return;
            Messages.safeSend(game.channel, { content: fill(MSG.day.timeWarning, { seconds: TIMING.discussionWarningBefore / 1000 }) });
        }, duration * 1000 - TIMING.discussionWarningBefore);
    }

    // 議論終了処理。①制限時間経過による自然終了 と ②/skip talk による早期停止 の両方をここで一本化して受ける
    let dayPhaseEnded = false;
    msgCollector.on('end', async (_collected: any, reason: string) => {
        if (dayPhaseEnded) return;
        dayPhaseEnded = true;

        game.isDiscussionActive = false;
        game.discussionCollector = null;
        if (warningTimer) clearTimeout(warningTimer);

        try {
            // /skip talk 経由の場合はスキップ成立アナウンスを既に送信済みのため、通常の終了メッセージは重複させない
            if (reason !== DISCUSSION_SKIP_REASON) {
...
/**
 * /skip talk コマンドのハンドラ①：投票の登録と成立判定のみを行う（Discordへの送信は一切行わない）。
 * 生存している人間プレイヤーの過半数が賛成した時点で `achieved: true` を返す。
 * 実際にフェーズを終了させる処理は `triggerDiscussionSkip` に分離してあるので、
 * 呼び出し側で「①この結果をreplyする → ②成立していればtriggerDiscussionSkipを呼ぶ」の順に必ず実行すること。
 * （①と②を同時に走らせると、投票フェーズ開始メッセージの方が「成立しました」replyより先に届いてしまう順序崩れが起きるため）
 */
export function registerDiscussionSkipVote(game: GameState, playerId: string): { success: boolean; message: string; achieved: boolean } {
    if (game.state !== 'playing' || !game.isDiscussionActive || !game.discussionCollector) {
        return { success: false, message: MSG.day.skipNotAvailable, achieved: false };
    }

    const player = game.players.find((p: Player) => p.id === playerId);
    if (!player || !player.alive || player.isNpc) {
        return { success: false, message: MSG.day.skipNotAvailable, achieved: false };
    }

    if (!game.discussionSkipVotes) game.discussionSkipVotes = [];
    if (game.discussionSkipVotes.includes(playerId)) {
        return { success: false, message: MSG.day.skipAlreadyVoted, achieved: false };
    }
    game.discussionSkipVotes.push(playerId);

    const aliveHumans = game.players.filter((p: Player) => p.alive && !p.isNpc).length;
    const needed = Math.floor(aliveHumans / 2) + 1;
    const current = game.discussionSkipVotes.length;

    if (current >= needed) {
        return { success: true, message: fill(MSG.day.skipAchieved, { current, needed }), achieved: true };
    }

    return { success: true, message: fill(MSG.day.skipRequested, { name: player.name, current, needed }), achieved: false };
}

/**
 * /skip talk コマンドのハンドラ②：実際に議論コレクターを停止し、投票フェーズへの移行を発火させる。
 * 必ず「成立しました」replyを送り終えた"後"に呼び出すこと（呼び出し順序の詳細は registerDiscussionSkipVote 参照）。
 */
export function triggerDiscussionSkip(game: GameState): void {
    const collector = game.discussionCollector;
    game.discussionCollector = null;
    try {
        collector?.stop(DISCUSSION_SKIP_REASON);
    } catch (e) {
        console.error('[triggerDiscussionSkip] collector.stop エラー:', e);
    }
}
...
export async function startMorningPhase(game: GameState, victimId: string | null, guardSuccess: boolean, extraVictims: string[] = []) { 
    ...
    // 📜 遺言モード: 夜に死亡した(襲撃で死んだ)プレイヤーにもDMで最期の言葉を求める。
    // 朝の通知を遅らせないよう、結果を待たずに非同期で流す（届き次第チャンネルへ追記投稿される）
    if (game.settings.willMode) {
        game.players
            .filter((p: Player) => p.deathReason === 'kill' && p.deathDay === game.dayCount)
            .forEach((v: Player) => collectNightWill(game, v));
    }
...
/**
 * 📜 遺言モード(willMode)用のヘルパー。夜に死亡した(襲撃された)プレイヤー1人につき呼び出す。
 * 処刑時の遺言収集（チャンネルでリアルタイム募集）とは異なり、夜のうちは死亡に気づけないため、
 * DMで個別に最期の言葉を尋ね、届き次第チャンネルへ追記投稿する非同期フローになっている。
 * 朝のメインアナウンスをブロックしないよう、呼び出し元では await せず fire-and-forget で使うこと。
 */
async function collectNightWill(game: GameState, victim: Player): Promise<void> {
    if (victim.isNpc) {
        const npcWill = MSG.npcWills[Math.floor(Math.random() * MSG.npcWills.length)];
        if (!game.chatLog) game.chatLog = [];
        game.chatLog.push({ id: victim.id, name: victim.name, content: `(遺言) ${npcWill}`, day: game.dayCount });
        if (!game.timeline) game.timeline = [];
        game.timeline.push({ type: 'chat', day: game.dayCount, id: victim.id, name: victim.name, content: npcWill, isWill: true });

        setSafeTimeout(game, () => {
            if (game.state !== 'playing') return;
            Messages.safeSend(game.channel, fill(MSG.morning.willReveal, { name: victim.name, will: npcWill }));
        }, TIMING.coronerDelayBase + Math.random() * TIMING.coronerDelayRandom);
        return;
    }

    if (!victim.user) return;
    const sent = await Messages.safeDM(victim.user, fill(MSG.morning.willRequest, { seconds: TIMING.willTimeLimit / 1000 }));
    if (!sent) return;

    try {
        const dmChannel = await victim.user.createDM();
        const collected = await dmChannel.awaitMessages({ filter: (m: any) => m.author.id === victim.id, max: 1, time: TIMING.willTimeLimit, errors: ['time'] });
        const willText = collected.first()!.content;

        if (!game.chatLog) game.chatLog = [];
        game.chatLog.push({ id: victim.id, name: victim.name, content: `(遺言) ${willText}`, day: game.dayCount });
        if (!game.timeline) game.timeline = [];
        game.timeline.push({ type: 'chat', day: game.dayCount, id: victim.id, name: victim.name, content: willText, isWill: true });

        if (game.state === 'playing') await Messages.safeSend(game.channel, fill(MSG.morning.willReveal, { name: victim.name, will: willText }));
    } catch {
        // タイムアウト・DM返信なし → 無言だった旨だけ静かに伝える
        if (game.state === 'playing') await Messages.safeSend(game.channel, fill(MSG.morning.willSilent, { name: victim.name }));
    }
}
