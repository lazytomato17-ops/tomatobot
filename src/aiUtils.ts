...
export async function generateNpcGaya(
    ...
    // 💡 修正ポイント：真面目な推理を強制せず、「キャラの個性を最優先しろ」と強く指示
    const messages = [
        { 
...
3. 直前の【会話ログ】にしっかり耳を傾け、無視せずに「相槌」「反論」「質問への回答」などを行い、自然な会話のキャッチボールを成立させること。
4. 【超重要】あなたの「性格・口調」の設定を何よりも最優先で発言に反映させること。
5. 【超重要】全員が真面目に推理するとつまらないです。「サンズ」「ジェビル」「ジャックス」「中二病」などの特殊な性格の場合、無理に誰かを疑ったり議論を進めたりする必要は全くありません。自分の世界観やダジャレ、メタ発言などを全開にして、純粋に場をかき乱すエンターテイナーになってください。` 
        },
        { 
            role: 'user' as const, 
...
上記の状況を踏まえ、あなたの強烈な個性を100%発揮して発言してください。
真面目なキャラ（normal, serious等）は推理を、ふざけたキャラ（sans, jevil, jax等）はメタ発言や場を荒らす発言を最優先に出力してください。` 
        }
    ];
    ...
    try {
        const chatCompletion = await groq.chat.completions.create({
            ...
            temperature: 0.9, // 💡 少しだけ温度（ランダム性）を上げてカオス度をアップ！
            max_tokens: 80, 
        });
