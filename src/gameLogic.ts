...
const PRESET_LABELS: Record<string, string> = {
    ...
    preset_random_balanced: '【ランダム】バランス重視',
    preset_random:          '【ランダム】完全カオス村',
    preset_dark_pot:        '【闇鍋モード】',
};
            ...
            if (interaction.customId.startsWith('lobby_preset')) {
                ...
                if (preset.startsWith('load_preset_')) {
                    ...
                    Object.assign(game.settings, {
                        ...
                        firstNightPeace: true, matchType: 'casual', isDarkPot: false,
                    });
                } else if (preset === 'preset_random_balanced') {
                    // ⚖️ 【新設】バランス重視ランダム（完全枠固定型）
                    const newRoles: string[] = [];

                    // ① 村の必須級（占い・霊能・騎士）からランダムに2個選出（被りなし）
                    const coreVillage = ['seer', 'medium', 'guard'].sort(() => 0.5 - Math.random());
                    newRoles.push(...coreVillage.slice(0, 2));

                    // ② 村の追加スパイス（最大1つ）
                    const subVillage = ['baker', 'cat', 'tough_guy', 'mayor', 'compass', 'freemason'].sort(() => 0.5 - Math.random());
                    newRoles.push(subVillage[0]);

                    // ③ 人狼のサポーター（狂人・狂信者・妖術師から【必ず1つだけ】）
                    const wolfSupporters = ['madman', 'fanatic', 'sorcerer'].sort(() => 0.5 - Math.random());
                    newRoles.push(wolfSupporters[0]);

                    // ④ 闇のスパイス（第三陣営など、30%の確率で1つだけ混入させる）
                    if (Math.random() < 0.3) {
                        const thirdFactions = ['fox', 'cupid', 'devotee', 'thief', 'teruteru'].sort(() => 0.5 - Math.random());
                        newRoles.push(thirdFactions[0]);
                    }

                    // 重複を排除して配列化
                    const randomRoles = Array.from(new Set(newRoles));

                    Object.assign(game.settings, {
                        roles: randomRoles, wolfMode: 'auto', continuousGuard: false,
                        tieVoteHandling: 'revote', voteTransparency: 'anonymous',
                        firstNightPeace: true, matchType: 'casual', isDarkPot: false,
                    });
                    
                    let roleSlots = randomRoles.length;
                    if (randomRoles.includes('freemason')) roleSlots++; // 共有者は2枠消費
                    targetTotal = roleSlots + 2; // 必要人数の目安を計算

        } else if (preset === 'preset_dark_pot') {
            // 🍲 【改修】闇鍋モード（バランスを調整したカオス村）
            
            // 人狼側が有利になりすぎないように、人狼陣営の特殊役職を定義
            const wolfTeamSubRoles = ['madman', 'fanatic', 'sorcerer', 'divider', 'loquacious'];
            
            // それ以外の役職（seerは確定で入れるため除外）
            const otherIngredients = [
                'medium', 'guard', 'freemason', 'coroner', 'tough_guy', 'cat',
                'fox', 'fugitive', 'cupid', 'devotee', 'assassin', 
                'compass', 'wolf_possessed', 'cursed'
            ];

            // 闇鍋の時は必ず「占い師」を含める
            const darkPotRoles = ['seer']; 
            
            // 合計5〜8個の役職になるように、残りの枠（4〜7個）を決定
            const pickCount = Math.floor(Math.random() * 4) + 4;
            
            // 全体をシャッフルする
            const allIngredients = [...wolfTeamSubRoles, ...otherIngredients].sort(() => Math.random() - 0.5);
            
            let wolfSubCount = 0;
            for (const role of allIngredients) {
                if (darkPotRoles.length >= pickCount + 1) break; // 目標数（占い師＋追加枠）に到達したら終了
                
                // 人狼陣営の特殊役職が多すぎると村が崩壊するため、最大2つまでに制限
                if (wolfTeamSubRoles.includes(role)) {
                    if (wolfSubCount >= 2) continue;
                    wolfSubCount++;
                }
                
                darkPotRoles.push(role);
            }

            Object.assign(game.settings, {
                roles: darkPotRoles,
                wolfMode: 'auto',
                continuousGuard: Math.random() > 0.5, // ルールすら闇
                tieVoteHandling: 'random',
                voteTransparency: 'anonymous',
                firstNightPeace: true,
                matchType: 'casual',
                isDarkPot: true // 💥 闇鍋フラグON
            });
            let roleSlots = darkPotRoles.length;
            if (darkPotRoles.includes('freemason')) roleSlots++; 
            targetTotal = roleSlots + 2; 
                    ...
                    const pickCount = Math.floor(Math.random() * 3) + 5; 
                    const randomRoles = shuffled.slice(0, pickCount);

                    Object.assign(game.settings, {
                        roles: randomRoles, wolfMode: 'auto', continuousGuard: true,
                        tieVoteHandling: 'random', voteTransparency: 'anonymous',
                        firstNightPeace: true, matchType: 'casual', isDarkPot: false,
                    });
...
async function startGame(game: GameState, interaction: any) {
    ...
    // 🍲 闇鍋モードなら内訳テキストを隠蔽する
    const displayBreakdown = game.settings.isDarkPot 
        ? "？？？ (何が潜んでいるかは完全な闇の中……)" 
        : roleBreakdown;

    // あなたの元のコード構造を一切破壊しません
    ...
    const wolfTeamIds = game.players
        .filter((p: Player) => !p.isNpc && (Roles.isActualWolf(p.role as string) || p.role === '分断者' || p.role === '狂信者'))
        .map((p: Player) => p.id);
