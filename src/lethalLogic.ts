...
function getGame(channelId: string): GameState | undefined { return activeGames.get(channelId); }
...
function getMoonRow(game: GameState) {
    ...
    // 3段目: モニター班＆船の「システム」
    const sysRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    return [moveRow, actionRow, sysRow]; // 最大5個までなら3段でもスッキリ！
}
...
export async function handleExplore(interaction: any, direction: 'left' | 'forward' | 'right') {
    ...
    try {
        ...
        if (direction === 'left') {
            dangerModifier = +20; scrapMulti = 1.6; dirLabel = "左の通路（ハイリスク）"; // 左は危険だが儲かる
        } else if (direction === 'right') {
            dangerModifier = -15; scrapMulti = 0.7; successBase -= 20; dirLabel = "右の通路（安全）"; // 右は安全だがハズレが多い
        } else {
            dirLabel = "正面の扉（標準）"; // 前は普通
        }
...
export async function handleMonitor(interaction: any) {
    ...
    try {
        ...
        const embed = new EmbedBuilder().setAuthor({ name: getStatusHeader(game) }).setTitle('💻 モニター室からの通信').setDescription(`**【${player.name} のレーダー解析結果】**\n*${await generateDescription('Scan', dText)}*`).setColor(game.facilityDanger > 70 ? 0xFF0000 : 0x00FF00);
        await interaction.editReply({ embeds: [embed], components: getMoonRow(game) });
    } finally { game.isProcessing = false; }
}
...
export async function handleTeleport(interaction: any) {
    const game = getGame(interaction.channelId)!;
    const player = game.players.get(interaction.user.id);
    if (!player || player.role !== 'monitor') return interaction.reply({ content: '❌ モニター班のみ操作可能です。', ephemeral: true });
    const target = Array.from(game.players.values()).find(p => p.role === 'scavenger' && p.isAlive);
    if (!target) return interaction.reply({ content: '⚠️ 現場に生存者がいません。', ephemeral: true });

    await prepareNewMessage(interaction);
    game.activeEncounter = null; 
    const lostItem = target.inventory; target.inventory = 0; target.hasTwoHanded = false; target.hp = Math.min(100, target.hp + 20); 
    
    const embed = new EmbedBuilder().setAuthor({ name: getStatusHeader(game) }).setTitle('🌀 強制テレポート作動').setDescription(`**${player.name}** の操作により、**${target.name}** を船内へ強制転送しました。\n\n命は助かりましたが、所持していたスクラップ（${lostItem}円分）は全て失われました。${getPlayerStatusLine(target)}`).setColor(0x3498db);
    await interaction.editReply({ embeds: [embed], components: getMoonRow(game) });
}
...
export async function handleReturn(interaction: any, isAuto = false) {
    ...
    let total = 0; game.players.forEach(p => { if (p.isAlive) { total += p.inventory; p.inventory = 0; p.hasTwoHanded = false; } });
    game.funds += total; game.day += 1; game.location = 'orbit'; 
...
export async function handleBuy(interaction: any, item: 'flashlight' | 'shovel') {
    ...
    await interaction.reply({ content: `✅ **${item === 'flashlight' ? '🔦懐中電灯' : '⛏️シャベル'}** を購入しました！(残金: ${game.funds}円)` });
}