...
export const infoCommand = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        const { data: party, error } = await supabase.from('poke_caught_pokemons').select('*').eq('owner_id', interaction.user.id).eq('is_party', true);
        ...
        try {
            ...
            // 実数値の計算（レベル、種族値、個体値から算出。※努力値と性格補正は簡略化）
            const lv = poke.level;
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    { name: '❤️ HP', value: `${poke.current_hp} / ${realHp} \`(個体値: ${poke.iv_hp})\``, inline: false },
