    ...
    setSafeTimeout(game, async () => {
        for (const med of announcers) {
            try {
                let isBlack = false;
                let shouldReveal = true;
                let currentTargetId = game.lastExecutionResult?.id;

                if (med.role === '霊能者') {
                    if (game.lastExecutionResult) isBlack = game.lastExecutionResult.isWolf;
                    if (med.isNpc) {
                        if (med.isHiding) {
                            let forceReveal = isBlack || game.dayCount >= 3;
                            if (forceReveal) med.isHiding = false; 
                ...
                if (currentTargetId) {
                    ...
                    if (!existingEv) {
                        ...
                        game.evidence.push({ type: 'medium_co', day: game.dayCount, from: med.id, target: currentTargetId, result: isBlack, visible: shouldReveal });
                    }
                }
                ...
                if (hiddenLogs.length > 0) {
                    ...
                    hiddenLogs.forEach((e: any) => { 
                        if (e.target) {
                            const tName = game.players.find((p: Player) => p.id === e.target)?.name || '不明';
                            pastResults += `\n・${e.day - 1}日目の処刑者 **${tName}** ➔ **【${e.result ? '人狼🐺' : '人間👤'}】**`; 
                        }
                    });
                ...
                if (!game.chatLog) game.chatLog = [];
                if (!game.timeline) game.timeline = [];
                game.chatLog.push({ id: med.id, name: med.name, content: currentTargetId ? `霊媒結果: ${executedPlayer?.name} は ${isBlack ? '黒' : '白'}` : `霊能CO`, day: game.dayCount });
                game.timeline.push({ type: 'chat', day: game.dayCount, id: med.id, name: med.name, content: currentTargetId ? `霊媒結果: ${executedPlayer?.name} は ${isBlack ? '黒' : '白'}` : `霊能CO` });
            ...
            if (game.lastExecutionResult) {
                ...
                mainComponents = [
                    new ActionRowBuilder<ButtonBuilder>().addComponents(
                        new ButtonBuilder().setCustomId('strategy_co').setLabel('📢 朝にCOして発表').setStyle(ButtonStyle.Primary),
                        new ButtonBuilder().setCustomId('strategy_hide').setLabel('🥷 潜伏する').setStyle(ButtonStyle.Secondary)
                    )
                ];
            } else {
                mainContent = `👻 **霊能者**\n昨晩は処刑が行われなかったため、霊能結果はありません。\n（※必要に応じて「潜伏」や「CO」の方針をあらかじめ設定できます）`;
                mainComponents = [
                    new ActionRowBuilder<ButtonBuilder>().addComponents(
                        new ButtonBuilder().setCustomId('strategy_co').setLabel('📢 朝にCOする').setStyle(ButtonStyle.Primary),
                        new ButtonBuilder().setCustomId('strategy_hide').setLabel('🥷 潜伏する').setStyle(ButtonStyle.Secondary)
                    )
                ];
            }
