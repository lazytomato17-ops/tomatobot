...
import { getMovesForLevel } from './pokeApiUtils'; // 👈 レベルアップ時の技習得用に追加
...
interface BattlePokemon {
    ...
    imageUrl: string; moves: BattleMove[]; types: string[];
...
async function buildBattlePokemon(dbPoke: any): Promise<BattlePokemon> {
    // ステータス計算と画像URL取得のため最低限のAPI通信（技の大量フェッチは不要）
    ...
    return {
        ...
        moves: dbPoke.moves, // 👈 DBから直接読み込み
        types: dbPoke.types, // 👈 DBから直接読み込み
        exp: dbPoke.exp || 0
    };
}
...
export async function startBattle(interaction: MessageComponentInteraction, challengerId: string, targetId: string) {
    ...
    } catch (e) { console.error(e); await interaction.followUp('バトル開始エラー'); }
}
        ...
        if (defPoke.hp === 0) {
            ...
            try {
                ...
                await supabase.from('poke_caught_pokemons').update({ 
                    level: currentLevel, exp: currentExp, moves: atkPoke.moves, types: atkPoke.types,
                    pokedex_id: atkPoke.pokedexId, nickname: atkPoke.nickname 
                }).eq('id', atkPoke.dbId);
            ...
            if (nextIdx === -1) {
                ...
                try {
                    ...
                    await supabase.from('poke_users').update({ 
                        money: (u?.money || 0) + 500, wins: (u?.wins || 0) + 1, 
                        win_streak: newStreak, max_win_streak: newMaxStreak 
...
async function updateBattleMessage(interaction: MessageComponentInteraction, battleId: string, isFinished = false) {
    ...
    const p1Alive = b.p1.party.filter(p => p.hp > 0).length;
    const p2Alive = b.p2.party.filter(p => p.hp > 0).length;

    const embed = new EmbedBuilder().setTitle(isFinished ? '🏁 終了' : '⚔️ バトル進行中').setDescription(b.log).setColor(isFinished ? 0x808080 : 0xFF4500)
        .addFields(
            { name: `🔵 相手: <@${b.p2.id}>`, value: `**${p2p.nickname}** Lv.${p2p.level} HP:${p2p.hp}/${p2p.maxHp} (残り${p2Alive}匹)`, inline: false },
            { name: `🔴 自分: <@${b.p1.id}>`, value: `**${p1p.nickname}** Lv.${p1p.level} HP:${p1p.hp}/${p1p.maxHp} (残り${p1Alive}匹)`, inline: false }
