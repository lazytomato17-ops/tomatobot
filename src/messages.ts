...
function buildSettingsSummary(settings: any): string {
    ...
    return [
        `**設定**：${matchText} (${settings.discussionTime}秒 / 狼: ${wolfText})`,
        `**役職**：${rolesDisplay}`, // 💡 ここで上記の分岐テキストをそのまま流し込みます
        `**追加**：${ruleChips.length > 0 ? ruleChips.join('、') : 'なし'}`
    ].join('\n');
}
...
export async function getLobbyPayload(game: GameState, userId: string, member?: any) {
    ...
    // --- 役職設定から「最低必要な人数」を計算 ---
    const baseWolves = game.settings.wolfMode === 'auto' ? 1 : (typeof game.settings.wolfMode === 'number' ? game.settings.wolfMode : 1);
    ...
    if (humanPlayers.length === 0 && npcCount === 0) {
        ...
        if (npcCount > 0) all.push(`🤖 *NPC (${npcCount}名)*`);
    ...
    const lobbyColor = game.settings.matchType === 'ranked' ? COLORS.LOBBY_RANKED : COLORS.LOBBY_CASUAL;
    ...
    const presetOptions: any[] = [
        { label: '📋 スタンダード',          value: 'preset_standard',        description: '基本役職の固定編成（初心者向け）' },
        { label: '⚖️ バランスランダム',       value: 'preset_random_balanced', description: '基本役職＋スパイス役職で事故のないランダム' },
        { label: '🎲 完全カオスランダム',     value: 'preset_random',          description: '全役職から完全ランダム（上級者向け）' },
        { label: '🍲 闇鍋モード',            value: 'preset_dark_pot',        description: '内訳完全非公開！配られた自分の役職だけが真実' },
    ];
...
export function getSettingsComponents(settings: any, currentTab: string = 'basic', isPremium: boolean = false) {
    ...
    if (currentTab === 'basic') {
        const matchOptions = [
            ...
            { label: '🏆 ランクマッチ (レート変動あり)', value: 'ranked', default: settings.matchType === 'ranked', description: '人間2名以上必須' },
        ];
        const wolfOptions = [
        ...
        const advancedOptions = [
            ...
            { label: '📜 遺言モード (最期の言葉を残せる)', value: 'will', default: settings.willMode, description: '処刑・襲撃で死亡したプレイヤーが最期の言葉を残せるようになる' },
        ];
