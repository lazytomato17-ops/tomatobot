...
- **Dockerビルドが `npm error code 127` で失敗する不具合を修正**。本番イメージ作成時（`npm ci --omit=dev`）は `husky` が devDependencies のためインストールされず、`package.json` の `prepare` スクリプトが `husky: not found` で失敗しビルド全体が止まっていた。`npm ci` に `--ignore-scripts` を付け、ライフサイクルスクリプトをコンテナ内で実行しないように変更（依存パッケージはどれも postinstall 等に依存していないことを確認済み）。あわせて `prepare` スクリプト自体も `husky || true` とし、`.git` が無い環境（CI/Dockerビルド時）でも失敗しないようガード
- Node.js のバージョン要件を 20 → **22** に引き上げ（`lint-staged` 最新版が Node 22 以上を要求するため。Dockerfile・CI・`package.json` の `engines` を統一）
